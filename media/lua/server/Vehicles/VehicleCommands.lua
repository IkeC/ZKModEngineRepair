...
function Commands.repairEngine(player, args)
	...
	if vehicle then
		...
		local done = 0
		local numberOfPartsToUse = args.numberOfParts
		...
		for i=1,numberOfPartsToUse do
			local newPartCondition = part:getCondition() + ZKCondPerPart
			ZKERPrint("new part condition: " .. part:getCondition() .. " + " .. ZKCondPerPart .. " = " .. newPartCondition)
			part:setCondition(newPartCondition)
